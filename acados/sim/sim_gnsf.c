                              ...
                              external_function_generic *get_matrices_fun)
{
    ...
    void *ext_fun_in[1];
    ext_fun_arg_t ext_fun_type_out[11];
    void *ext_fun_out[11];
                         ...
                         void *mem_, void *work_, double T)
{
    ...
    blasfeo_dgemv_n(nyy, nZ1, 1.0, &LLZ, 0, 0, &ZZ0, 0, 1.0, &YY0, 0, &YY0, 0);
                                                // YY0 = YY0 + LLZ * ZZ0
    // build M2_inv
    blasfeo_ddiare(nK2, 1.0, &M2inv, 0, 0);     // set M2inv to eye(nK2)
    // blasfeo_ddiare(nK2, 1.0, &M2, 0, 0);     // add eye(nK2) to M2;
    for (int ii = 0; ii < num_stages; ii++)
    {
        blasfeo_pack_dmat(nxz2, nxz2, model->E_LO, nx2+nz2, &M2, ii * nxz2, ii * nxz2);
        for (int jj = 0; jj < num_stages; jj++)
        {
...
int sim_gnsf_workspace_calculate_size(void *config, void *dims_, void *opts_)
{
    ...
    size += blasfeo_memsize_dvec(nxz2);  // ALOtimesx02

    size += blasfeo_memsize_dvec(nuhat);  // uhat

    if (opts->sens_algebraic){
        size += blasfeo_memsize_dvec(nx1);  // x0dot_1;
        size += blasfeo_memsize_dvec(nz1);  // z0_1;
    ...
    size += blasfeo_memsize_dmat(nK2, nvv);  // aux_G2_vv
...
static void *sim_gnsf_cast_workspace(void *config, void *dims_, void *opts_, void *raw_memory)
{
    ...
    if (opts->sens_algebraic){
        ...
        assign_and_advance_blasfeo_dvec_mem(nz1, &workspace->z0_1, &c_ptr);
    ...
    assign_and_advance_blasfeo_dmat_mem(nK2, nvv, &workspace->aux_G2_vv, &c_ptr);
...
int sim_gnsf(void *config, sim_in *in, sim_out *out, void *args, void *mem_, void *work_)
{
    ...
    struct blasfeo_dmat S_forw_new = workspace->S_forw_new;  // used to avoid side evvects
    ...
    struct blasfeo_dmat aux_G2_vv = workspace->aux_G2_vv;
    ...
    struct blasfeo_dvec z0_1 = workspace->z0_1;
    ...
    {  // STEP LOOP
        ...
        if (nxz2)
        {
            ...
            blasfeo_dgemv_n(nxz2, nx2, 1.0, &ALO, 0, 0, &x0_traj, ss * nx + nx1, 0.0, &f_LO_val,
                            0, &ALOtimesx02, 0);
                    // ALOtimesx02 = ALO * x2part(x0_traj(step));
            for (int ii = 0; ii < num_stages; ii++)
            {  // Evaluate f_LO + jacobians
...
/* output z and propagate corresponding sensitivities */
        ...
        /* propagate sensitivities of z1 */
            if (opts->sens_algebraic)
            {
                ...
                // pack z0_1
                blasfeo_pack_dvec(nz1, out->zn, &z0_1, 0);

                // evaluate phi at x0_1, x0_1dot, u, z0_1;
                ...
                blasfeo_dgemv_n(ny, nx1, 1.0, Lx, 0, 0, &x0_traj, 0, 1.0,
                                &y_one_stage, 0, &y_one_stage, 0);
                // y_one_stage += Lz * z0_1
                blasfeo_dgemv_n(ny, nz1, 1.0, Lz, 0, 0, &z0_1, 0, 1.0,
                                &y_one_stage, 0, &y_one_stage, 0);
 ...
 ************************************************/
    ...
    if (opts->sens_adj)
    {
        for (int ss = num_steps - 1; ss >= 0; ss--)
        {
            ...
            acados_tic(&la_timer);
            blasfeo_dgemm_nn(nK2, nvv, nK2, -1.0, &M2inv, 0, 0, &aux_G2_vv, 0, 0, 0.0, &dK2_dvv, 0,
                             0, &dK2_dvv, 0, 0);
